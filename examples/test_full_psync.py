...
class Producer(object):
    def __init__(self, face, keyChain, syncPrefix, userPrefixUri, nDataPrefixes,
      maxNPublished):
        self._face = face
        self._fullPSync = FullPSync2017(
          80, self._face, syncPrefix, self.processSyncUpdate, keyChain, 1600, 1600)
        self._nDataPrefixes = nDataPrefixes
        self._maxNPublished = maxNPublished
        self._delayRangeMaxMs = 6000.0
        # The key is the Name. The value is the int nPublished
        self._nPublished = {}

        # Add name prefixes and schedule updates for them in specified the interval.
        for i in range(self._nDataPrefixes):
            prefixUri = userPrefixUri + "-" + str(i)
            dataPrefix = Name(prefixUri)

            self._nPublished[dataPrefix] = 0
    ...
    def doUpdate(self, dataPrefix):
        name = Name(dataPrefix).appendVersion(int(Common.getNowMilliseconds()))

        dump("Publish:", name.toUri())
        self._fullPSync.publishName(name)

        self._nPublished[dataPrefix] += 1

        if self._nPublished[dataPrefix] < self._maxNPublished:
            # random1 is from 0.0 to 1.0.
            random1 = self._systemRandom.random()
            delay = random1 * self._delayRangeMaxMs
